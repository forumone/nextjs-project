...
async function readExistingNames() {
  let contents;
  try {
    contents = await readFile(spritePath, 'utf8');
  } catch {
    return null;
  }

  const names = new Set();
  for (const match of contents.matchAll(/<symbol id="([^"]+)"/g)) {
    names.add(match[1]);
  }
  return names;
}

async function isSpriteStale(filenames) {
...
async function generateSprite() {
  ...
  if (!force && !(await isSpriteStale(filenames))) {
    console.log('✅ Icon sprite is up to date, skipping regeneration.');
    return;
  }
  ...
  console.log(`🖌️ Wrote ${symbols.length} icons to ${spritePath}`);
}
